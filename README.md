# Moving-average-backtest-in-Nepalese-market
...
sma20_nepse_ts<-Lag(
  ifelse(Lag(Cl(Nepse_MA1))<Lag(sma20_nepse) & 
           Cl(Nepse_MA1)>sma20_nepse,1,
         ifelse(Lag(Cl(Nepse_MA1))>Lag(sma20_nepse)&
                  Cl(Nepse_MA1)< sma20_nepse,-1,0)))
sma20_nepse_ts[is.na(sma20_nepse_ts)]<-0
...
sma_nepse_ts<-Lag(ifelse(Lag(sma20_nepse)< Lag(sma50_nepse)&
               sma20_nepse>sma50_nepse,1,
             ifelse(Lag(sma20_nepse)>Lag(sma50_nepse)&
                      sma20_nepse<sma50_nepse,-1,0)))
...
sma_Nepse_stratcomp<-cbind(sma20_nepse,sma50_nepse,
                           sma_nepse_ts, sma_Nepse_strat)
colnames(sma_Nepse_stratcomp)<-c("SMA(20)","SMA(50)",
...
charts.PerformanceSummary(sma_Nepse_comp,
                          main="Nepse VS SMA crossover Performance")
